...
fn looper_state_color(state: LooperMode) -> Color {
    match state {
        ...
        LooperMode::Ready => Color::from_rgb(150, 255, 0),
        LooperMode::None => Color::from_rgb(135, 135, 135),
    }
}

fn looper_state_dark_color(state: LooperMode) -> Color {
    match state {
        LooperMode::Record => Color::from_rgb(210, 45, 45),
        LooperMode::Overdub => Color::from_rgb(0, 255, 255),
        LooperMode::Playing => Color::from_rgb(0, 213, 0),
        LooperMode::Ready => Color::from_rgb(100, 200, 0),
...
impl UI {
    ...
    pub fn draw(&mut self, canvas: &mut Canvas, data: &Engine) {
        ...
        // draw play head bar
        let beat = data.metric_structure.tempo.beat(FrameTime(data.time));
        let bom = data.metric_structure.time_signature.beat_of_measure(beat);
        ...
        if bom == 0 {
            ...
            let v = self.beat_animation.as_ref().unwrap().value(FrameTime(data.time));
...
impl BottomBarView {
    ...
    fn draw(&mut self, canvas: &mut Canvas, _w: f32, h: f32, data: &Engine) {
        let time = FrameTime(data.time);
        ...
        canvas.draw_str(
            &format!("{} bpm", data.metric_structure.tempo.bpm() as u32),
        ...
        let current_beat = data.metric_structure.tempo.beat(time);
        let beat_of_measure = data.metric_structure.time_signature.beat_of_measure(current_beat);
        let measure = data.metric_structure.time_signature.measure(current_beat);
        for beat in 0..data.metric_structure.time_signature.upper {
            let mut paint = Paint::default();
            paint.set_anti_alias(true);
...
impl LooperView {
    ...
    fn draw(&mut self, canvas: &mut Canvas, data: &Engine, index: usize) {
        let looper = &data.loopers[index];

        let ratio = (data.time.rem_euclid(looper.length_in_samples() as i64)) as f32 /
            looper.length_in_samples() as f32;
        draw_circle_indicator(canvas, looper_state_color(looper.mode), ratio, 25.0, 25.0, 25.0);
...
fn path_for_channel(looper: &Looper, channel: usize, w: f32, h: f32, flip: bool) -> Vec<(f32, f32)> {
    let mut d = vec![0f32; looper.length_in_samples() as usize];

    let sample_count = looper.samples.len() as f32;
    for s in &looper.samples {
        for i in 0..looper.length_in_samples() as usize {
            d[i] += s.buffer[channel][i] / sample_count;
        }
    }
...
impl WaveformView {
    ...
    fn draw(&mut self, canvas: &mut Canvas, data: &Engine, index: usize, w: f32, h: f32) {
        let looper = &data.loopers[index];
        ...
        let full_w = (looper.length_in_samples() as f32 / self.time_width.0 as f32) * w;

        if self.path.is_none() {
            let p_l = path_for_channel(looper, 0, full_w, h - 10.0, true);
            let p_r = path_for_channel(looper, 1, full_w, h - 10.0, false);
        ...
        if self.bar_lines.is_none() {
            ...
            let samples_per_beat = FrameTime::from_ms(1000.0 /
                (data.metric_structure.tempo.bpm() / 60.0) as f64);
            let number_of_beats = looper.length_in_samples() / samples_per_beat.0 as u64;
            for i in 0..number_of_beats {
                let x = i as f32 * full_w / number_of_beats as f32;

                if i % data.metric_structure.time_signature.upper as u64 == 0 {
        ...
        paint.set_color(looper_state_dark_color(looper.mode));
        ...
        let mut x = -self.time_to_x(FrameTime(data.time), w);
        while x < w * 2.0 {
            if x + full_w > 0.0 && x < w {
                canvas.save();
